            ...
            $.get(g5_bbs_url+"/ajax.autosavelist.php", function(data){
                ...
                if ($(data).find("list").find("item").length > 0) {
                    $(data).find("list").find("item").each(function(i) {
                        ...
                        $("#autosave_pop ul")
                            .append('<li class="list-group-item"><a href="#none" class="autosave_load"><span class="text-muted">['+datetime+']</span> '+subject+'</a> <button type="button" class="autosave_del btn btn-xs btn-white pull-right"><i class="fa fa-times"></i></button></li>')
                            .find("li:eq("+i+")")
