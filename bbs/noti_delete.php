...
if($p_type == "del" || $p_type == "read"){
	for($i=0;$i<count($_POST['chk_bn_id']);$i++) {
        ...
        if(count($tmp_ph_id) > 0){
            ...
            if($p_type == "del"){ // 삭제
                sql_query(" delete from ".$g5['na_noti']." where ph_id in (".$group_ph_id.") ", false);
            } else if($p_type == "read"){ // 읽음
                sql_query(" update ".$g5['na_noti']." set ph_readed = 'Y' where ph_id in (".$group_ph_id.") ", false);
            }
        }
	}
	...
	sql_query(" delete from ".$g5['na_noti']." where mb_id = '".$member['mb_id']."' ", false);
...
goto_url("./noti.php?page=$page&read=$read");
