...
$g_ids = preg_replace('/[^0-9a-z\,]/i', '', $g_ids);
...
$result = sql_query(" select ph_id, mb_id, bo_table, wr_parent, ph_to_case, ph_from_case, rel_url from ".$g5['na_noti']." where ph_id in (".$g_ids.") ", false);
...
if(count($tmp_ph_id) > 0){
	...
	sql_query(" update ".$g5['na_noti']." set ph_readed = 'Y' where ph_id in (".$group_ph_id.") $tmp_add_sql ", false);
